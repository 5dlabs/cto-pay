...
3. Implement pages:
   ...
   - **`/agents`:** Browse all registered AgentPackage accounts. Display card grid: package_id, author (truncated pubkey), split_bps as percentage, total_earned (USDC), task_count, success_rate (success_count/task_count * 100%), active badge, source_uri as Arweave link (`ar://` → `https://arweave.net/txid`), content_hash (truncated hex with copy button). Sort by total_earned or success_rate.
   - **`/agents/[packageId]`:** Agent detail page with full stats, content_hash verification (fetch from Arweave, compute SHA-256, compare to on-chain hash), earnings chart (placeholder for future), and task history.
